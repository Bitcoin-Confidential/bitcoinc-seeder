...
echo "SEED=${SEED}
SEEDER=${SEEDER}
EMAIL=${EMAIL}" > /etc/.bitcoinc-seeder-config
...
ln -sf "$script_dir"/bitcoinc-seeder /usr/local/bin
systemctl enable "$script_dir"/bitcoinc-seeder.service

# Start the seeder
systemctl start bitcoinc-seeder
